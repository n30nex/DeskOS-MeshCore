...
- Use in this project: tracked compatibility/BSP reference for board support, LCD/touch integration patterns, and D1L/GX display configuration. The included `components/LoRaWAN/soft-se/aes.c` carries the permissive Brian Gladman notice embedded in `aes.c` and `aes.h`. DeskOS pins its own production ESP-IDF version independently of the upstream example baseline.
...
- Use in this project: MeshCore's pinned Ed25519 verifier remains the API and implementation base. Production replaces only `fe.c`, `ge.c`, and `sc.c` with the plainly marked SIGUI defined-arithmetic overlay under `overlays/meshcore_ed25519_defined/`.
...
- Additional reference checkout reviewed at commit: `9ae0a3d03a1fcb0a6e77844ae97f0bfc5bab7f1e`.
...
- Permission: the maintainers gave this project permission to use the work with attribution; the maintainers retain the permission record.
- Use in this project: reference architecture for a production touch-first MeshCore desk firmware, especially UI shell structure, navigation patterns, cached input state, storage/map concepts, packet/log visibility, release tooling, and validation ideas. No literal SigurdOS source is copied into this D1L slice.
...
- Additional reference checkout reviewed at commit: `519238f9084fe5b20e18716794a8d48a895ee6c9`.
- This project is used as a reference only; no literal source is copied.
