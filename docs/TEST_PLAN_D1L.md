...
- Map fetch, background-prefetch, and signed-location contract: canonical surfaces are `map|map_options|map_location|map_cache`. OpenStreetMap Standard remains the fixed interactive current-view source: the saved center opens at default zoom 10; direct `-`/`+` controls clamp to zooms 8 through 14, `Center` returns to the saved location, a one-finger drag commits only on release, and a visible generation requests at most one 3x3 at one zoom. The separate SD-authorized provider in `map/offline-provider.json` may prefetch only when the Map is hidden, configured Wi-Fi is connected, the persistent SD file/atomic-rename gate is ready, a saved device location exists, and the provider explicitly grants HTTPS network fetch, offline storage, and background prefetch. The deterministic plan bounds the device plus retained valid node adverts no farther than 200 km from the device, handles the antimeridian, completes lower zooms first, and selects the highest provider zoom that fits both the card reserve and configured `cache_budget_mb` (default 18432 MB; accepted range 256 through 24576 MB). Changing device location, retained marker generation, provider policy/budget, or storage generation replans; opening Map pauses the background worker and gives the interactive view priority. Provider rate limits, HTTPS, attribution, and the built-in OSM no-bulk restriction remain fail closed.

  Every committed tile is tracked in a provider-local, checksummed FIFO journal. Before a network request, the cache reserves the maximum tile size against the configured byte budget; when space is required it advances the oldest journal records deterministically and deletes only their derived tile and tile-metadata paths, never node/contact/history files or provider configuration. A download streams only to `.tmp`, validates PNG content, reads the SD temporary file back to verify its CRC32, writes checksummed temporary tile metadata, appends a durable commit intent, then atomically renames the tile and metadata before atomically advancing the checksummed cache state. On reboot, a valid uncommitted journal tail resumes from the verified temporary or final tile; corrupt state, record, metadata, content, or an unrecoverable partial commit fails closed instead of accepting or overwriting it. Cached reads recheck PNG structure and the recorded CRC32. Legacy untracked tiles are treated as misses and replaced through this tracked path. The cache never writes tiles to NVS and the eviction path is tile-only.
