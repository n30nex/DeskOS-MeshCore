...
- Inbound-DM ACK admission and deferred-persistence contract: schema v7 retains the full 32-byte message identity digest plus per-row reservation count, route kind, lifecycle state, and last error; valid v1-v4 rows remain visible as `legacy_unverified` but never hydrate collision-safe ACK dedupe state. A newly authenticated DM must first enter the bounded in-RAM ring, and capacity or nonterminal-eviction failure must emit no ACK RF. Its dispatch count and `pending` route kind must then be reserved in RAM before the ACK enters the priority queue. The radio owner must perform no SD/NVS flush while admitting, reserving, queueing, completing, timing out, or rolling back an inbound ACK; the retained worker later persists the latest revision. Therefore `durable=false` is truthful during that volatile window: power loss before the worker pass can lose the new row and dispatch budget, and no receipt may describe the ACK as storage-durable until a clean worker pass is observed. The first in-RAM attempt may use the selected direct path and the sole retry may fall back to flood, optionally with PATH; no admitted identity may exceed two physical ACK dispatches in the active boot, and the ceiling survives reboot only after that state has been persisted. Callback correlation uses the full digest as primary identity and treats a row sequence only as a stale-safe hint. Boot normalization emits zero RF. TxDone, timeout, and queue failure must publish truthful `sent`, `retryable`, or `terminal` RAM state and error before deferred persistence. Host contracts must prove there is no blocking store call before ACK queueing, duplicate suppression while `pending`, full-ring rejection before RF, deferred completion/rollback, worker convergence, same-digest/different-sequence reconciliation, higher-epoch resequencing, stale callbacks, reboot after a completed worker pass, malformed legacy data, and schema migration. Historical PR #98 evidence banks the superseded durability-first baseline only; exact-candidate ACK latency, worker convergence, reboot-after-flush, controlled power-loss behavior, and peer/RF evidence remain separate gates for this deferred contract.
...
- Map fetch, background-prefetch, and signed-location contract: canonical surfaces are `map|map_options|map_location|map_cache`. OpenStreetMap Standard remains the fixed interactive current-view source: the saved center opens at default zoom 10; direct `-`/`+` controls clamp to zooms 8 through 14, `Center` returns to the saved location, a one-finger drag commits only on release, and a visible generation requests at most one 3x3 at one zoom. The separate SD-authorized provider in `map/offline-provider.json` may prefetch only when the Map is hidden, configured Wi-Fi is connected, the persistent SD file/atomic-rename gate is ready, a saved device location exists, and the provider explicitly grants HTTPS network fetch, offline storage, and background prefetch. The deterministic plan bounds the device plus retained valid node adverts no farther than 200 km from the device, handles the antimeridian, completes every lower/mid zoom that fits, then spends the remaining card-safe `cache_budget_mb` (default 18432 MB; accepted range 256 through 24576 MB) through a gap-free zoom ladder of disjoint child tiles owned by the device, geographically spread retained-node clusters, and the most recent viewport. Changing device location, retained marker generation, provider policy/budget, storage generation, or retained viewport replans; opening Map pauses the background worker and gives the interactive view priority. Provider rate limits, HTTPS, attribution, and the built-in OSM no-bulk restriction remain fail closed.
  ...
  Every committed tile is tracked in a provider-local, checksummed FIFO journal under one bounded foreground/background transaction mutex. A download streams only to `.tmp`, validates PNG content, reads the SD temporary file back to verify its CRC32, and only then reserves its actual validated size; when space is required it advances the oldest journal records deterministically and deletes only their derived tile and tile-metadata paths, never node/contact/history files or provider configuration. The commit writes checksummed temporary tile metadata, appends a durable intent, then atomically renames the tile and metadata before atomically advancing the checksummed cache state. A lost append acknowledgement forces immediate on-disk reconciliation rather than retrying a stale sequence. On reboot, a valid uncommitted journal tail resumes from the verified temporary or final tile, while a torn or duplicate uncommitted tail is atomically rebuilt to its valid complete prefix without locking out the cache; corruption inside the committed prefix still fails closed. Cached reads recheck PNG structure and the recorded CRC32. Legacy untracked tiles are treated as misses and replaced through this tracked path. The cache never writes tiles to NVS and the eviction path is tile-only.

  If the complete persistent-file gate is not ready when a physically opened Map begins, that same visible generation waits on cached storage status with zero tile attempts or network requests and resumes automatically when SD becomes ready; hiding or covering Map cancels the visible request. Drag motion does not request tiles. Passive marker refresh queries at most 32 newest located nodes and displays at most eight. Only a verified, nonfuture MeshCore signed advert with a valid location may supply a pin; the detail label says `Advert location`, reports verified age and exact known role, retains unknown accuracy, and never infers expiry, local GPS, or precision. Initial entry, interactive pan/zoom, and Center reacquisition use explicit center provenance and fail closed if absent or invalid. Trust loss invalidates the retained view and releases the lease; backward wall-time correction rechecks future pins. Markers remain bright non-red role-aware dots with readable below-marker names, skip deterministic collisions/control/attribution exclusions, and update only their lightweight layer without acquiring a tile generation or copying/repainting the tile frame. The marker layer follows drag preview and reprojects only after committed pan/zoom. A viewport-level hit radius of 22 px opens Node Detail by retained fingerprint; Close reacquires the unchanged retained Map view without a new intentional download. Run `python -m pytest -q tests/test_map_marker_truth.py tests/test_map_point_projection.py tests/test_map_viewport_ui_contract.py tests/test_map_prefetch_plan.py tests/test_map_prefetch_service_contract.py tests/test_map_tile_cache_policy.py`. A completed exact-view Home-to-Map revisit retains frame revision/generation and leaves attempted/cache-hit/download/network counters unchanged, with no SD read operation in the trace. A reboot may reread the checksummed SD cache but must not redownload valid cached tiles. Permanent attribution remains visible. Scroll probes never request map tiles and do not prove background acceptance. This software gate does not close exact-candidate provider, SD, reboot/offline-cache, nearby-pin rendering, or physical UI acceptance.
...
For the current contact-targeted TRACE software regression, run `python -m pytest --cache-clear -q tests/test_meshcore_trace_contract.py tests/test_meshcore_trace_native.py tests/test_meshcore_admin_dispatch_contract.py tests/test_meshcore_route_selection_contract.py tests/test_meshcore_oracle_contract.py::test_oracle_manifest_is_exactly_pinned_and_fail_closed`. Acceptance requires request `flags=0` for one-byte contact hashes or `flags=1` for two-byte contact hashes, generic wire parsing/building for pinned `flags=[0,1,2,3]` / `trace_wire_hash_bytes_supported=[1,2,4,8]`, exact prefix-preserving normalization of three-byte contact routes to two-byte TRACE hashes, one pending request, a 30-second timeout, a 60-second duplicate window, correlation on tag plus the opaque correlation code plus the exact immutable runtime-derived loop and hash width, immediate Admin-session invalidation on direct-response derivation failure, fixed `trace_last` retention, `contact_trace_supported=true`, `operator_path_accepted=false`, `one_byte_hash_only=false`, `contact_trace_path_hash_bytes_supported=[1,2]`, and `hardware_verified=false`. This host check sends no RF and cannot close physical or official-peer gates.
...
5. Run `routes probe <fingerprint>` only when an opt-in DM RF trace is allowed. Verify the response has `cmd="routes probe"`, `queued=true`, a generated `path_` token, `dm_rf_tx=true`, and `public_rf_tx=false`.
...
16. For one quick live Map acceptance, set a location, connect Wi-Fi, then open the actual Map. Verify it opens at zoom 10 with permanent attribution visible. Drag with one finger and prove the image previews during motion but no tile plan/request begins until release; then verify at most nine tiles for that one newly visible view. Exercise `-` and `+` through zooms 8 and 14, confirm the controls stop at those bounds, and verify each tap creates at most one current-view 3x3 plan at one zoom per visible generation. Tap `Center` and verify the saved coordinates are restored. Leave Map during an unfinished visible batch and verify its remaining requests stop. After one view is complete, go Home and return to Map; require the same frame generation/revision and unchanged attempted/cache-hit/download/network counters, plus no SD read operation in the trace, so the retained frame appears without network or SD reread. Power-cycle, reopen that view, and verify the SD tile cache may be reread but valid cached tiles are not downloaded again. Then run `python .\scripts\scroll_probe_d1l.py --port <D1L_PORT> --screens map,map_options,map_location,map_cache`; verify its network-suppressed before/after rows have equal `network_requests`, `network_tx=false`, `map_network_requests=false`, `background_download=false`, `area_download=false`, `visible_tile_limit=9`, and `zoom_batch_limit=1`.

    Separately, with the authorized SD provider installed, the saved location set, nearby retained located nodes present, Map closed, and Wi-Fi connected, record the background status transition through queued/running to complete. Require every valid retained node within 200 km in the planned bounds, no farther node, the configured budget and selected zoom range, deterministic tile-only eviction when a deliberately small test budget is used, permanent attribution, `public_rf_tx=false`, and `formats_sd=false`. Interrupt one tile after its checksummed intent is durable but before both final renames, reboot, and require recovery of that exact tile without accepting corrupt content or issuing a second download. Disconnect Wi-Fi and revisit the node area from cache; reboot once and revisit again, proving no cached tile is redownloaded and the retained node list remains until explicit `nodes clear`. No timed soak is part of Map acceptance. Exact-candidate evidence must name the firmware SHA, Actions run/artifact, D1L stable port, SD/provider manifest hash, budget, tile counts/bytes, interruption point, reboot identity, network-request deltas, and every rendered nearby pin.
