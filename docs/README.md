# SIGUI documentation

## Current release truth

- [`AGENTS.md`](../AGENTS.md) — repository operating rules.
- [`USER_GUIDE_D1L.md`](USER_GUIDE_D1L.md) - current 1.5 full-feature product guide.
- [`KNOWN_LIMITATIONS.md`](KNOWN_LIMITATIONS.md) - current 1.5 product limits.
- [`RC2_SCOPE.md`](RC2_SCOPE.md) - historical corrective 1.2 product contract.
- [`RC1_SCOPE.md`](RC1_SCOPE.md) — historical 1.0/RC1 product contract.
- [`ROADMAP.md`](ROADMAP.md) — shipped 1.0/RC1, corrective 1.2/RC2 and deferred
  1.5/RC3 in one release queue.
- [`DESKOS_MESHCORE_FEATURE_PARITY.md`](DESKOS_MESHCORE_FEATURE_PARITY.md) —
  completed Android/iOS parity ledger inherited by 1.5.
- [`RC1_RELEASE_EXECUTION_D1L.md`](RC1_RELEASE_EXECUTION_D1L.md) — historical
  1.0 maintainer publication record.
- [`RELEASE_CHECKLIST.md`](RELEASE_CHECKLIST.md) — completed 1.0/RC1 and
  1.2/RC2 publication records.

## User documentation

- [`README.md`](../README.md) - product and download entrypoint.
- [`USER_GUIDE_D1L.md`](USER_GUIDE_D1L.md) — normal product workflows.
- [`KNOWN_LIMITATIONS.md`](KNOWN_LIMITATIONS.md) — current user-visible limits.
- [`D1L_SD_CARD_GUIDED_INSTALL.md`](D1L_SD_CARD_GUIDED_INSTALL.md) — safe SD
  preparation.
- [`ADMIN_REMOTE_CLI_ALLOWLIST.md`](ADMIN_REMOTE_CLI_ALLOWLIST.md) — supported
  admin commands.
- [`ATTRIBUTIONS.md`](ATTRIBUTIONS.md) — third-party attribution.
- [`screenshots/DEVICE_1_2_CAPTURE.md`](screenshots/DEVICE_1_2_CAPTURE.md) —
  attached-device 1.2 screenshot provenance and framebuffer CRCs.

## Developer-only material

- [`TEST_PLAN_D1L.md`](TEST_PLAN_D1L.md) describes internal development and CI
  checks. It is not part of the public package or publication checklist.
- [`RC3_BACKLOG.md`](RC3_BACKLOG.md) records the feature and debt sources folded
  into the 1.5 release.

## Historical archive

Archived planning and completion evidence are provenance only. They must not
be executed or treated as current release instructions.

- [`archive/pre-rc1-authority-reset/`](archive/pre-rc1-authority-reset/README.md)
- [`completion/`](completion/README.md)
